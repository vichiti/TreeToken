    ...
    <v-row class="mt-4">
      <v-col>
        <v-card>
          ...
          <v-card-text>Total: ${{ (cartTotal * 1 + cartTotal * 0.02).toFixed(2) }} (incl. 2¢ fee)</v-card-text>
          <v-card-actions>
...
export default {
  ...
  setup() {
    ...
    const checkout = () => {
      alert(`Paying for ${cartTotal.value} TTs!`);
      cart.value = [];
      cartTotal.value = 0;
    };
